...
| "fx_chroma_distort" | 一个废弃的fx，效果为带色散的扭曲    |                                             |
| "fx_film"           | 老电影效果                | 0不启用，1启用                                    |
| "boost_distance"    | 让note靠近判定线时加速下落      | 0为无效果，值越大开始加速的距离离判定线越远；<br>如果是负数会让note往上飞   |
| "boost_time"        | 控制boost_distance的加速度 | 值越大加速度越大，小于等于0不生效                           |
| "hom"               | 轨道重影                 | 使用时proxies不能为-1；0为无重影，值越大，重影越明显，大于1后就没什么变化了 |
| "enable_hue"        | 开启Hue滤镜              | [仅在csm2.3+有效]<br>1为开启，0关闭                   |
...
| 可用mod        | 作用                       | 说明                                                           |
|:------------:|:------------------------:|:------------------------------------------------------------:|
| "spinradius" | 用于控制spinProxies/swirl的幅度 | mpf中有spinProxies/swirl才生效                                    |
| "spiny"      | 已废弃                      |                                                              |
| "spinx"      | 已废弃                      |                                                              |
| "prx"        | 调整轨道的x坐标                 | 默认为0<br>实际轨道x位置=prx+prxb+prxc+prxd                           |
| "prxb"       | 调整轨道的x坐标                 | 默认为0                                                         |
| "prxc"       | 调整轨道的x坐标                 | 默认为0                                                         |
| "prxd"       | 调整轨道的x坐标                 | 默认为0                                                         |
| "pry"        | 调整轨道的y坐标                 | 默认为0,<br>实际轨道y位置=pry+pryb+pryc+pryd                          |
| "pryb"       | 调整轨道的y坐标                 | 默认为0                                                         |
| "pryc"       | 调整轨道的y坐标                 | 默认为0                                                         |
| "pryd"       | 调整轨道的y坐标                 | 默认为0                                                         |
...
| "prcl"       | 将ui遮盖向右移动                | 控制绘制范围的最左侧的x坐标<br>只有使用了这个或prcr才能带上ui<br>prcl与prcr值都为0.35时不生效 |
| "prcr"       | 将ui遮盖向右移动                | 控制绘制范围的最右侧的x坐标<br>只有使用了这个或prcl才能带上ui<br>prcl与prcr值都为0.35时不生效 |
| "prvib"      | 用于控制quake效果的强度           | mpf中有quake的才有效                                               |
...
<br>
各个obj的独有gimmick请见附上的 <a href="./Extra Gimmicks(NO COMMENTS).xlsx">Extra Gimmicks表格</a>
